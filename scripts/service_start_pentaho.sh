...
if [ ! -f "$PENTAHO_HOME/pentaho_bi_installed_version.txt" ]; then 
  ...
  if [ -d "$PENTAHO_HOME/server/biserver-ee/start-pentaho.sh" ]; then
	...
	/sbin/setuser pentaho /scripts/install_pentaho_ba_5_4_0_1-130.sh
	#/sbin/setuser pentaho /scripts/upgrade_pentaho_ba.sh #- Implemtent this later.
  fi
else
  #Pentaho is installed.  Check which version is installed.
  PENTAHO_BA_INSTALLED_VER=$(grep -oP 'biserver-ee-\K[\d\.\-]*' $PENTAHO_HOME/pentaho_bi_installed_version.txt)
