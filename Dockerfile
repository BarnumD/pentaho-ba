...
#
#  #On first ever run, you must include the PGPWD (postgres adimn) password to initialize the database.  Subsequent runs should not need this in the ENV
#   docker run -i --name pentaho-ba-test -p 8080:8080 -e PGUSER=postgresadm -e PGPWD=8s88jjjChangeMe99aks88 -e PGHOST=pentaho-db-test -e PGPORT=5432 -e PGDATABASE=postgres --link pentaho-db-test:pentaho-db-test -v /docker/mounts/pentaho-ba-test/opt/pentaho:/opt/pentaho pentaho-ba:latest &
#
#  #On subsequent boots, (after db is initialized), run non-interactive.
#   #test
#    docker run -d --name pentaho-ba-test -p 8080:8080 -e TIER=TEST -e PGUSER=postgresadm -e PGHOST=pentaho-db-test -e PGPORT=5432 -e PGDATABASE=postgres --link pentaho-db-test:pentaho-db-test -v /docker/mounts/td_t1/pentaho-ba-test/opt/pentaho:/opt/pentaho --memory 6344M --memory-swap -1 --oom-kill-disable -c=512 pentaho-ba:latest &
#   #Prod
#    docker run -d --name pentaho-ba-prod -p 8080:8080 -e TIER=PROD -e PGUSER=postgresadm -e PGHOST=pentaho-db-prod -e PGPORT=5432 -e PGDATABASE=postgres --link pentaho-db-prod:pentaho-db-prod -v /docker/mounts/p_t1/pentaho-ba-prod/opt/pentaho:/opt/pentaho --memory 6656M --memory-swap -1 --oom-kill-disable -c=1024 pentaho-ba:0.56 &
#
...
ENV PENTAHO_BA_TARGET_VER="5.4.0.3-143"
